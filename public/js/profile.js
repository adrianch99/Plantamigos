...
let profileUser = null, currentUser = null, isOwnProfile = false;
...
function renderProfile() {
  ...
  if (u.avatar_url) {
    avatarEl.innerHTML = `<img src="${u.avatar_url}" class="profile-avatar" alt="${u.name}"><button class="profile-edit-avatar" id="btn-edit-avatar" title="Cambiar foto"></button>`;
  } else {
    const letter = (u.name || '?')[0].toUpperCase();
    avatarEl.innerHTML = `<div class="profile-avatar" style="display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#22c55e,#16a34a);font-size:2.5rem;font-weight:700;color:#000;">${letter}</div><button class="profile-edit-avatar" id="btn-edit-avatar" title="Cambiar foto">📷</button>`;
  }
  document.getElementById('profile-name').textContent = u.name;
  document.getElementById('profile-location').textContent = [u.neighborhood, u.city].filter(Boolean).join(', ') || '📍 Sin ubicación';
  ...
  document.getElementById('profile-since').textContent = new Date(u.created_at).toLocaleDateString('es', { year: 'numeric', month: 'long' });
  ...
  if (isOwnProfile) {
    ...
    document.getElementById('btn-contact-user').style.display = currentUser ? 'flex' : 'none';
  }
}
...
function setupEventListeners() {
  ...
  document.getElementById('form-edit-profile').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const stored = getUser(); Object.assign(stored, updated);
  ...
  document.addEventListener('click', (e) => {
    if (e.target.id === 'btn-edit-avatar' || e.target.closest('#btn-edit-avatar') || e.target.id === 'btn-upload-profile-pic') {
  ...
  document.getElementById('btn-plant-image').addEventListener('click', () => {
    openCloudinaryWidget(CLOUD_NAME, UPLOAD_PRESET, (url) => {
      ...
      document.getElementById('plant-image-preview').innerHTML = `<img src="${url}" style="max-height:100px;border-radius:8px;">`;
    });
  });
  document.getElementById('form-add-plant').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      renderPlants();
      document.getElementById('stat-plants').textContent = profileUser.plants.length;
